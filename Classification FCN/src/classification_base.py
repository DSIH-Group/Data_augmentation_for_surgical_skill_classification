...
def inject_synthetic_data(x_real_windows, y_real_windows, current_path):
    """
    Routes the correct synthetic data into the training pool to prevent LOUO leakage.
    Uses the 'subdir' path to determine which Expert is currently held out.
    """
    
    # Check if Expert 1 (3_Out) is the test set
    if "3_Out" in current_path:  
        print(f"\n--- Fold 3_Out detected (Expert 1 held out) ---")
        print("Injecting ONLY Expert 2 synthetic data...")
        synthetic_x = np.load('../results-expert-3000iter-d_out-kt/synthetic_jigsaws_data.npy')
        
    # Check if Expert 2 (4_Out) is the test set
    elif "4_Out" in current_path: 
        print(f"\n--- Fold 4_Out detected (Expert 2 held out) ---")
        print("Injecting ONLY Expert 1 synthetic data...")
        synthetic_x = np.load('../results-expert-3000iter-e_out-kt/synthetic_jigsaws_data.npy')
        
    # Any other fold (Novices/Intermediates held out)
    else:
        print(f"\n--- Fold {current_path.split('/')[-2]} detected ---")
        print("Injecting ALL Expert synthetic data...")
        syn_1 = np.load('../results-expert-3000iter-d_out-kt/synthetic_jigsaws_data.npy')
        syn_2 = np.load('../results-expert-3000iter-e_out-kt/synthetic_jigsaws_data.npy')
        synthetic_x = np.concatenate((syn_1, syn_2), axis=0)

    # Create the labels ('E' for Expert)
    synthetic_y = ['E'] * len(synthetic_x)

    # Merge the Real and Synthetic Data
    x_train_combined = np.concatenate((x_real_windows, synthetic_x), axis=0)
    y_train_combined = y_real_windows + synthetic_y
    ...
    for it in range(1):
        for subdir, dirs, files in os.walk(path):
            ...
            if (len(files) > 0):
                ...
                #x_train_windows, y_train_windows = inject_synthetic_data(x_train_raw, y_train_raw, subdir)
...
def fitModel(model, x_train, y_train, x_test, y_test, x_val, y_val, out_dir, shuff=True, val_split=False):
    ...
    for epoch in range(0, nb_epochs):
        ...
        for sequence, label in zip(x_train, y_train_binary):
            # Pass the single, full-length trial through the 20-stream splitter
            loss, acc = model.train_on_batch(
                x=split_input_for_training(sequence), 
                y=label.reshape(1, nb_classes)
            )
            epoch_train_loss += loss  

        # Average the loss over the total number of full trials
...
out_root_dir = '../results/Needle_Passing/classification-base/'
nb_epochs = 1000
max_iterations = 1
