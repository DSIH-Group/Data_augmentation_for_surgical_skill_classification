...
def inject_synthetic_data(x_real_windows, y_real_windows, current_path, surgery):
    """
    Routes the correct synthetic data into the training pool to prevent LOUO leakage.
    Accumulates data safely without overwriting.
    """
    dataset = '../LOUO/'+surgery
    
    # 1. Initialize accumulators with the real data so we don't overwrite it
    x_train_combined = np.array(x_real_windows)
    y_train_combined = list(y_real_windows)
    
    # 2. Define exactly which 3 GANs are safe to use for the current hold-out
    if "1_Out" in current_path:   # Test: B (Novice)
        safe_folds = ['ALL_INTERMEDIATES', 'ALL_EXPERTS']
    elif "2_Out" in current_path: # Test: C (Intermediate)
        safe_folds = ['C_OUT', 'ALL_EXPERTS']
    elif "3_Out" in current_path: # Test: D (Expert)
        safe_folds = ['ALL_INTERMEDIATES', 'D_OUT']
    elif "4_Out" in current_path: # Test: E (Expert)
        safe_folds = ['ALL_INTERMEDIATES', 'E_OUT']
    elif "5_Out" in current_path: # Test: F (Intermediate)
        safe_folds = ['F_OUT', 'ALL_EXPERTS']
    elif "6_Out" in current_path: # Test: G (Novice)
        safe_folds = ['ALL_INTERMEDIATES', 'ALL_EXPERTS']
    elif "7_Out" in current_path: # Test: H (Novice)
        safe_folds = ['ALL_INTERMEDIATES', 'ALL_EXPERTS']
    else:                         # Test: I (Novice) -> 8_Out
        safe_folds = ['ALL_INTERMEDIATES', 'ALL_EXPERTS']

    print(f"\n--- Injecting synthetic data ---")
    print(f"Safe GANs selected: {safe_folds}")

    # 3. Iterate ONLY through the safe folders
    for fold in safe_folds:
        # Assign the correct class label based on the folder name
        if fold in ['ALL_EXPERTS', 'D_OUT', 'E_OUT']:
            label = 'E'
        elif fold in ['ALL_INTERMEDIATES', 'C_OUT', 'F_OUT']:
            label = 'I'
            
        # os.path.join handles the slashes correctly (e.g., ../LOUO/B_OUT/*.npy)
        npy_files = glob.glob(os.path.join(dataset, fold, '*.npy'))
        
        for file in npy_files:
            synthetic_x = np.load(file)
            synthetic_y = [label] * len(synthetic_x)
            
            # ACCUMULATE the data by combining it with the ongoing x_train_combined
            x_train_combined = np.concatenate((x_train_combined, synthetic_x), axis=0)
            y_train_combined = y_train_combined + synthetic_y
...
def convertStringClassesToBinaryClasses(y_train, y_test, y_val):
    ...
    y_train_test_val = to_categorical(y_train_test_val)
...
def shuffle(x_train, y_train):
    ...
    return x_shuffled, y_shuffled.tolist()
...
def validation(surgery_type='Suturing', balanced='Balanced', shuff=True,
               ...
               levelClassify=False, val_split=False, augment='timewarp', surgery='Needle_Passing'):
    ...
    for it in range(1):
        for subdir, dirs, files in os.walk(path):
            ...
            for file_name in files:
                ...
                surgeries_set = set()
                for gesture in data:
                    word_to_search = gesture[0] if type(gesture) is np.ndarray else gesture
                    surgery_name = find_pattern(word_to_search, surgery_type + '_.00.')
            ...
            if (len(files) > 0):
                ...
                x_train_raw, y_train_raw = extract_windows(x_train, y_train)
                if(augment == 'synthetic'):
                    #inject synthetic data
                    x_train_windows, y_train_windows = inject_synthetic_data(x_train_raw, y_train_raw, subdir, surgery)
                else:
                    #use tsaug
                    print('Using TSAug')
                    x_train_warped = TimeWarp().augment(x_train_raw)
                    x_train_windows = np.concatenate((x_train_raw, x_train_warped), axis=0)
                    y_train_windows = y_train_raw + y_train_raw

                fit_encoder(y_train_windows, y_test, y_val)
                ...
                for itrr in range(max_iterations):
                    ...
                    model.save(out_dir + 'model_init.keras')

                    epochs_loss, y_test_binary, val_loss = fitModel(model, x_train_windows, y_train_windows,
                                                                    x_test, y_test, x_val,
                                                                    y_val, out_dir, shuff=shuff, val_split=val_split)
                    
                    print(epochs_loss)

                    model = load_model(out_dir + 'model_best.keras')
...
def find_pattern(word, pattern):
    match = re.search(r'' + pattern, str(word))
    if match:
    ...
    for epoch in range(0, nb_epochs):
        ...
        for i in range(len(input_shapes)):
            for j in range(len(input_shapes[i])):
                dim_size = input_shapes[i][j][1]
                # Extract the exact block of features (e.g., 3, 9, 3, 3, 1)
                x_train_split.append(x_train[:, :, last : last + dim_size])
                last += dim_size
        
        # 1. Convert the list of 20 feature splits into a tuple so TensorFlow can digest it
        x_tuple = tuple(x_train_split)

        # 2. Build a high-performance streaming pipeline
        train_dataset = tf.data.Dataset.from_tensor_slices((x_tuple, y_train_binary))
        
        # 3. Batch the data and tell the CPU to prepare the next batch while the GPU is working
        train_dataset = train_dataset.shuffle(buffer_size=20000).batch(128).prefetch(tf.data.AUTOTUNE)

        # 4. Train the model using the stream
        history = model.fit(
            train_dataset, 
            epochs=1, 
            verbose=0, 
        ...
        epoch_train_loss = history.history['loss'][0]
        ...
        model.save(out_dir + 'model_curr.keras')
...
def evaluateModel(model, x_test, y_test_binary):
    ...
    for test, label in zip(x_test, y_test_binary):
        test_windows, _ = extract_windows([test], [label])
        test_windows_split = []
        last = 0
        for i in range(len(input_shapes)):
            for j in range(len(input_shapes[i])):
                dim_size = input_shapes[i][j][1]
                test_windows_split.append(test_windows[:, :, last : last + dim_size])
                last += dim_size
        p = model.predict(test_windows_split, batch_size=128, verbose=0)
        votes = np.argmax(p, axis=1).tolist()
        majority_vote = Counter(votes).most_common(1)[0][0]
        predicted_label = encoder.inverse_transform([majority_vote])[0]
        correct_label = encoder.inverse_transform([np.argmax(label)])[0]
        confusion_matrix.loc[correct_label, predicted_label] += 1.0
        confusion_matrix_f.loc[correct_label, predicted_label] += 1.0
...
def cas(idx_to_explain=0):
    ...
    model = keras.models.load_model('../results/classification/jigsaws-data/Experimental_setup/Knot_Tying/unBalanced/GestureClassification/UserOut/1_Out/itr_1/architecture__fcn/reg__1e-05/lr__0.001/filters__8/kernel_size__3/amsgrad__0/model_best.keras')
...
def results(validation='SuperTrialOut'):
    ...
    for surg in s_types:
        ...
        dir_out_surgery = out_root_dir + 'jigsaws-data/Experimental_setup/' + surg \
                        ...
                        for subdir, dirs, files in os.walk(curr_dir):
                            if len(files) > 0:
                                ...
                                if os.path.exists(file_name):
                                    ...
                                    # Dynamically search the path array for the exact hyperparameter folders
                                    curr_df['architecture'] = [x for x in arr if 'architecture__' in x][0].split('__')[1]
                                    curr_df['reg']          = [x for x in arr if 'reg__' in x][0].split('__')[1]
                                    curr_df['lr']           = [x for x in arr if 'lr__' in x][0].split('__')[1]
                                    curr_df['filters']      = [x for x in arr if 'filters__' in x][0].split('__')[1]
                                    curr_df['kernel_size']  = [x for x in arr if 'kernel_size__' in x][0].split('__')[1]
                                    curr_df['amsgrad']      = [x for x in arr if 'amsgrad__' in x][0].split('__')[1]
...
# Global parameters
root_dir = '../jigsaws-data/'
path_to_configurations = '../jigsaws-data/Experimental_setup/'
out_root_dir = '../results/Needle_Passing/classification-windowed-synthetic-exp-int-shuffled/'
nb_epochs = 1000
max_iterations = 1
...
if (len(sys.argv) > 1):
    ...
    for surgery_type in surgery_types:
        ...
        validation(surgery_type, balanced='unBalanced', validation='UserOut',
                   levelClassify=True,augment='synthetic', shuff=True, surgery='Needle_Passing')
