...
def timegan (ori_data, parameters, save_path=None, load_path=None):
  """TimeGAN function.
  ...
  def embedder (X, T):
    ...
    Returns:
      - H: embeddings
    """
    with tf.compat.v1.variable_scope("embedder", reuse = tf.AUTO_REUSE):
  ...
  def recovery (H, T):   
    ...
    """     
    with tf.compat.v1.variable_scope("recovery", reuse = tf.AUTO_REUSE):       
  ...
  def supervisor (H, T): 
    ...
    """          
    with tf.compat.v1.variable_scope("supervisor", reuse = tf.AUTO_REUSE):
